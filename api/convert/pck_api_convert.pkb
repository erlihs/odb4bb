CREATE OR REPLACE PACKAGE BODY pck_api_convert AS 

    -- XML<-->JSON
    -- Credits: https://github.com/mbleron/JSONUtilities
    ...
    function json_to_xmltype (input in clob)
    return xmltype
    is
    ...
    function xmltype_to_json (input in xmltype)
    return clob
    is
      doc     dbms_xmldom.DOMDocument;
    ...
    PROCEDURE xml_to_json (
        p_xml IN CLOB,
        r_json IN OUT NOCOPY CLOB
    ) AS
    BEGIN
        r_json := xmltype_to_json(XMLTYPE(p_xml));
    END; 

    PROCEDURE json_to_xml (
        p_json IN CLOB,
        r_xml IN OUT NOCOPY CLOB
    ) AS
    BEGIN
        r_xml := json_to_xmltype(p_json).getClobVal();
    END;
